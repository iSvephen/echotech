<script>
    ...
    function selectColumn(categoryId, column) {
        if (columnSelections[category.id] === column) {
            columnSelections[category.id] = null;
        } else {
            columnSelections[category.id] = column;
        }
        individualSelections[category.id] = {};
    }

    function toggleCell(categoryId, rowIndex, column) {
        if (columnSelections[category.id] !== null) {
            columnSelections[category.id] = null;
        }
        if (individualSelections[category.id][rowIndex] === column) {
            delete individualSelections[category.id][rowIndex];
        } else {
            individualSelections[category.id][rowIndex] = column;
        }
